...
pub fn structUI() void {
    ...
    {
        dvui.structEntryAlloc(@src(), dvui.currentWindow().gpa, Top, true, &Top.instance, .{ .margin = .{ .x = 10 } });
    }
...
pub fn themeEditor() void {
    ...
    const color_field_options = dvui.StructFieldOptions(dvui.Color, false){ .fields = .{
    ...
    dvui.structEntryEx(@src(), "dvui.Theme", dvui.Theme, false, dvui.themeGet(), .{
        .use_expander = false,
        .label_override = "",
        .fields = .{
            .name = .{ .disabled = true },
