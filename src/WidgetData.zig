...
pub fn minSizeSetAndRefresh(self: *const WidgetData) void {
    ...
    if (existing_min_size) |kv| {
        if (kv.value.used) {
            dvui.log.err("{s}:{d} id {x} already used this frame (highlighting), may need to pass .id_extra = <loop index> into Options (see https://github.com/david-vanderson/dvui/blob/master/readme-implementation.md#widget-ids )\n", .{ self.src.file, self.src.line, self.id });
            cw.debug_widget_id = self.id;
        }
    }
}
